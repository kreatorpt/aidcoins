...
#if defined(NDEBUG)
# error "Aidcoins cannot be compiled without assertions."
#endif
...
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Aidcoins: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Aidcoins Signed Message:\n";
...
{
    // Aidcoins: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 840000); // Aidcoins: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 3.5 * 24 * 60 * 60; // Aidcoins: 3.5 days
static const int64 nTargetSpacing = 2.5 * 60; // Aidcoins: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Aidcoins: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Aidcoins: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // Aidcoins: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        //txNew.vout[0].nValue = 50 * COIN; // kreatorpt: size of firstblock, premine must be here; default was 50 * COIN; 5000000000;
		
		// AID COIN PREMINE Explained:
		//  8.400.000 AID -> 0.01% for social solidarity institutions (first 84 registrations; 100.000 AID for each; 2% forced donation included) 
		//    840.000 AID -> 0.001% for AidCoin marketing (2% will also be donated); 
		txNew.vout[0].nValue = 9240000 * COIN;  // 9.240.000 AID is only 0.011% PREMINED for AidCoins project boost
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // Aidcoins: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("AidcoinsMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("AidcoinsMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static AidcoinsMiner(CWallet *pwallet)
{
    printf("AidcoinsMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("aidcoins-miner");
    ...
    try { loop {
        ...
        printf("Running AidcoinsMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("AidcoinsMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&AidcoinsMiner, pwallet));
}
